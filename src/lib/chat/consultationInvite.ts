...
function buildConsultationInviteFallback(userMessage: string): string {
...
/** Resolve invite for display after an API reply (not on welcome). */
export function resolveChatConsultationInvite(
  invite: string | null | undefined,
  userMessage: string,
  leadBotCompleted: boolean,
): string | null {
  if (leadBotCompleted) return null
  const trimmed = invite?.trim()
  if (trimmed) return trimmed
  return buildConsultationInviteFallback(userMessage)
}
