-- Extend resource_leads for cost planner downloads
...
ALTER TABLE public.resource_leads
  ...
  CHECK (resource_type IN ('visa_checklist', 'cost_planner'));

-- ---------------------------------------------------------------------------
-- submit_cost_planner_lead
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.submit_cost_planner_lead(
  p_first_name text,
  p_last_name text,
  p_email text,
...
REVOKE ALL ON FUNCTION public.submit_cost_planner_lead(text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_cost_planner_lead(text, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.submit_cost_planner_lead(text, text, text, text) TO authenticated;

-- ---------------------------------------------------------------------------
-- validate_cost_planner_access
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.validate_cost_planner_access(p_token uuid)
RETURNS boolean
LANGUAGE sql
SECURITY DEFINER
SET search_path = ''
...
AS $$
  SELECT EXISTS (
    ...
    WHERE access_token = p_token
      AND resource_type = 'cost_planner'
      AND token_expires_at > now()
  );
$$;

REVOKE ALL ON FUNCTION public.validate_cost_planner_access(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.validate_cost_planner_access(uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.validate_cost_planner_access(uuid) TO authenticated;
